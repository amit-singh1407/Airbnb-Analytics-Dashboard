...
const DashboardPage = () => {
    ...
    return (
        <div className="pb-10">
            {/* Hero Section */}
            <div className="glass-card mb-6 lg:mb-8 relative overflow-hidden p-4 lg:p-8">
                ...
                <div className="relative z-10 flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6">
                    <div className="flex-1">
                        <h1 className="text-2xl lg:text-3xl font-extrabold text-transparent bg-clip-text bg-linear-to-r from-white to-slate-400 mb-2">
                            Airbnb Analytics Dashboard
                        </h1>
                        <p className="text-slate-400 max-w-2xl text-xs lg:text-sm leading-relaxed">
                        ...
                        <div className="flex flex-wrap items-center gap-2 mt-4">
                            <span className="px-2 py-0.5 text-[10px] lg:text-xs font-medium bg-blue-500/10 text-blue-400 border border-blue-500/20 rounded-full">React (Vite)</span>
                            <span className="px-2 py-0.5 text-[10px] lg:text-xs font-medium bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 rounded-full">Tailwind CSS</span>
                            <span className="px-2 py-0.5 text-[10px] lg:text-xs font-medium bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 rounded-full">Flask</span>
                            <span className="px-2 py-0.5 text-[10px] lg:text-xs font-medium bg-pink-500/10 text-pink-400 border border-pink-500/20 rounded-full">Pandas</span>
                        </div>
                    </div>
                    
                    <div className="flex items-center gap-3 w-full xl:w-auto">
                        <button onClick={() => window.open('http://localhost:5000/api/export-csv', '_blank')} className="flex-1 xl:flex-none flex items-center justify-center gap-2 px-4 py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 rounded-lg transition-all text-xs lg:text-sm font-medium">
                            <Download size={16} />
                            Export CSV
                        </button>
                        <a href="https://github.com" target="_blank" rel="noopener noreferrer" className="flex-1 xl:flex-none flex items-center justify-center gap-2 px-4 py-2.5 bg-slate-200 hover:bg-white text-slate-900 rounded-lg transition-all text-xs lg:text-sm font-medium shadow-lg shadow-slate-200/10">
                            <Code size={16} />
                            GitHub
                        </a>
                    </div>
